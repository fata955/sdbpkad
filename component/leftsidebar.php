<!-- Left Sidebar -->
<?php
@session_start();
// Sisa kode Anda di sini
$level = $_SESSION['level'];
if ($level > 1){
?>

<aside id="leftsidebar" class="sidebar">
    <div class="navbar-brand">
        <button class="btn-menu ls-toggle-btn" type="button"><i class="zmdi zmdi-menu"></i></button>
        <a href="/"><img src="assets/images/logo.svg" width="25" alt="Aero"><span class="m-l-10">SIsdana</span></a>
    </div>
    <div class="menu">
        <ul class="list">
            <li>
                <div class="user-info">
                    <a class="image" href="profile.html"><img src="assets/images/profile_av.jpg" alt="User"></a>
                    <div class="detail">
                        <h4><?php echo ($_SESSION["lengkap"]);?></h4>
                        <small>Verifikasi</small>
                    </div>
                </div>
            </li>
            <li class="main"><a href="/"><i class="zmdi zmdi-home"></i><span>Dashboard</span></a></li>
            <li class="main"><a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-assignment"></i><span>Transaction</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/expenses">Tagihan SPM</a></li>
                </ul>
            </li>
            <li class="main"><a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-copy"></i><span>Report</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/reportincome">Tagihan SPM</a></li>
                </ul>
            </li>
            <li class="main"><a href="/change"><i class="zmdi zmdi-key"></i><span>Ganti Password</span></a></li>
            <li class="main"><a href="/logout"><i class="zmdi zmdi-close"></i><span>Close</span></a></li>
        </ul>
        
    </div>
</aside>

<?php
}else{


?>
<aside id="leftsidebar" class="sidebar">
    <div class="navbar-brand">
        <button class="btn-menu ls-toggle-btn" type="button"><i class="zmdi zmdi-menu"></i></button>
        <a href="index.html"><img src="assets/images/logo.svg" width="25" alt="Aero"><span class="m-l-10">SIsdana</span></a>
    </div>
    <div class="menu">
        <ul class="list">
            <li>
                <div class="user-info">
                    <a class="image" href="profile.html"><img src="assets/images/profile_av.jpg" alt="User"></a>
                    <div class="detail">
                        <h4><?php echo ($_SESSION["lengkap"]);?></h4>
                        <small>Super Admin</small>
                    </div>
                </div>
            </li>
            <li class="main"><a href="/"><i class="zmdi zmdi-home"></i><span>Dashboard</span></a></li>
            <li class="main"><a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-apps"></i><span>Master</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/skpd">Opd</a></li>
                    <li class="main menu"><a href="/sumberdana">Sumber Dana</a></li>
                    <li class="main menu"><a href="/bagsumberdana">Sub Sumber Dana</a></li>
                    <li class="main menu"><a href="/perubahan">Jenis Perubahan</a></li>
                </ul>
            </li>
            <li class="main"><a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-flower"></i><span>Settings</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/pagu">Pagu</a></li>
                    <li class="main menu"><a href="/sumberdanaopd">Sumber Dana</a></li>
                    <!-- <li><a href="/sdbpkad/perubahan">Perubahan</a></li> -->
                    <li class="main menu"><a href="icons-weather.html">User Management</a></li>
                </ul>
            </li>
            <li class="main"><a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-assignment"></i><span>Transaction</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/lacaksalur">Income</a></li>
                    <li class="main menu"><a href="/expenses">Expense</a></li>
                    <li class="main menu"><a href="/penguji">Entry Penguji</a></li>
                </ul>
            </li>
            <li class="main"><a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-copy"></i><span>Report</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/reportincome">Income</a></li>
                    <li class="main menu"><a href="blank.html">expenses</a></li>
                    <li class="main menu"><a href="blank.html">Sumber Dana</a></li>
                </ul>
            </li>
            <li class="main"> <a href="javascript:void(0);" class="menu-toggle"><i class="zmdi zmdi-assignment"></i><span>Mock API SIPD</span></a>
                <ul class="ml-menu">
                    <li class="main menu"><a href="/tagihandetail">SPM</a></li>
                    <li class="main menu"><a href="/sp2d">SP2D</a></li>
                    <li class="main menu"><a href="ticket-list.html">Relisasi Belanja</a></li>
                    <li class="main menu"><a href="ticket-detail.html">Realisasi Potongan</a></li>
                </ul>
            </li>
            <li class="main"><a href="/change"><i class="zmdi zmdi-key"></i><span>Ganti Password</span></a></li>
            <li class="main"><a href="/logout"><i class="zmdi zmdi-close"></i><span>Close</span></a></li>
        </ul>
        
    </div>
</aside>
<?php
}?>
