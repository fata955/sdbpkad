...
if ($_GET["action"] === "fetchVerif") {
  ...
  $sql = "SELECT a.nama_opd as skpd, (select z.nilai from pagu z where a.id=z.idopd) as pagu, (select COALESCE(sum(y.nilai_sumber),0) from t_opdsumberdana y where a.id=y.id_opd AND y.id_sumber='1') As daubg, (select COALESCE(sum(y.nilai_sumber),0) from t_opdsumberdana y where a.id=y.id_opd AND y.id_sumber='2') As pad from skpd a, pagu b where a.id=b.idopd";
...
if ($_GET["action"] === "filtertanggal") {
  ...
  if ($idopd > 1) {
    ...
    //menampilkan rekapan sumber dana
    $sql = "SELECT namasumberdana, sum(nilai_sumber) as nilai, sum(realisasi) as realisasinya, sum(sisa) as sisanya from (select c.id,a.namasubsumberdana,c.nilai_sumber,(SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(z.tanggal_spm) between '$start' AND '$end' AND z.id_spm=y.id_spm AND z.id_skpd='$opd' AND y.status=1) as realisasi,(SELECT COALESCE(sum(x.nilai_spm),0) from tspm x ,tspmsub w where Date(x.tanggal_spm) between '$start' AND '$end' AND x.id_spm=w.id_spm AND w.id_sumber > 0 AND x.id_skpd=b.id_sipd AND w.status=1) as totalrealisasi,b.nama_opd,c.nilai_sumber- (SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(z.tanggal_spm) between '$start' AND '$end' AND z.id_spm=y.id_spm AND y.id_sumber=c.id_subsumberdana AND z.id_skpd=$opd AND y.status=1) as sisa,(select sum(nilai_sumber) from t_opdsumberdana where id_opd=b.id ) as Pagu_Anggaran, f.namasumberdana FROM subssumber a, skpd b, t_opdsumberdana c, pagu d, t_perubahan e ,t_sumberdana f where c.id_opd=b.id AND c.id_subsumberdana=a.id AND c.id_opd=b.id AND d.idopd=c.id_opd AND a.idsumberdana=f.id AND e.id=c.id_perubahan AND e.status='AKTIF') as subquery group by namasumberdana";
    $result = mysqli_query($conn, $sql);
    $data2 = [];
    while ($row = mysqli_fetch_assoc($result)) {
      $data2[] = $row;
    ...
    $end = $conn->real_escape_string($end);
     ...
     //menampilkan rekapan sumber dana
     $sql = "SELECT namasumberdana, sum(nilai_sumber) as nilai, sum(realisasi) as realisasinya, sum(sisa) as sisanya from (select c.id,a.namasubsumberdana,c.nilai_sumber,(SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(z.tanggal_spm) between '$start' AND '$end' AND z.id_spm=y.id_spm AND  y.id_sumber=c.id_subsumberdana AND y.status=1 AND z.id_skpd='$opd') as realisasi,(SELECT COALESCE(sum(x.nilai_spm),0) from tspm x ,tspmsub w where Date(x.tanggal_spm) between '$start' AND '$end' AND x.id_spm=w.id_spm AND w.id_sumber > 0 AND x.id_skpd=b.id_sipd AND w.status=1) as totalrealisasi,b.nama_opd,c.nilai_sumber- (SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(z.tanggal_spm) between '$start' AND '$end' AND z.id_spm=y.id_spm AND y.id_sumber=c.id_subsumberdana AND z.id_skpd=b.id_sipd AND y.status=1) as sisa,(select sum(nilai_sumber) from t_opdsumberdana where id_opd=b.id ) as Pagu_Anggaran, f.namasumberdana FROM subssumber a, skpd b, t_opdsumberdana c, pagu d, t_perubahan e ,t_sumberdana f where c.id_opd=b.id AND c.id_subsumberdana=a.id AND c.id_opd=b.id AND d.idopd=c.id_opd AND a.idsumberdana=f.id AND e.id=c.id_perubahan AND e.status='AKTIF') as subquery group by namasumberdana";
     $result = mysqli_query($conn, $sql);
     $data2 = [];
     while ($row = mysqli_fetch_assoc($result)) {
       $data2[] = $row;
...
if ($_GET["action"] === "filtertglverif") {
  ...
  if ($idopd > 1) {
    ...
    //menampilkan rekapan sumber dana
    $sql = "SELECT namasumberdana, sum(nilai_sumber) as nilai, sum(realisasi) as realisasinya, sum(sisa) as sisanya from (select c.id,a.namasubsumberdana,c.nilai_sumber,(SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(y.updateby) between '$start' AND '$end' AND z.id_spm=y.id_spm AND  y.id_sumber=c.id_subsumberdana AND z.id_skpd='$opd' AND y.status=1) as realisasi,(SELECT COALESCE(sum(x.nilai_spm),0) from tspm x ,tspmsub w where Date(w.updateby) between '$start' AND '$end' AND x.id_spm=w.id_spm AND w.id_sumber > 0 AND x.id_skpd='$opd' AND w.status=1) as totalrealisasi,b.nama_opd,c.nilai_sumber-(SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(y.updateby) between '$start' AND '$end' AND z.id_spm=y.id_spm AND y.id_sumber=c.id_subsumberdana AND z.id_skpd='$opd' AND y.status=1) as sisa,(select sum(nilai_sumber) from t_opdsumberdana where id_opd=b.id ) as Pagu_Anggaran, f.namasumberdana FROM subssumber a, skpd b, t_opdsumberdana c, pagu d, t_perubahan e ,t_sumberdana f where c.id_opd=b.id AND c.id_subsumberdana=a.id AND c.id_opd=b.id AND d.idopd=c.id_opd AND a.idsumberdana=f.id AND e.id=c.id_perubahan AND e.status='AKTIF') as subquery group by namasumberdana";
    $result = mysqli_query($conn, $sql);
    $data2 = [];
    while ($row = mysqli_fetch_assoc($result)) {
      $data2[] = $row;
    ...
    // echo $start;
    // $sql = "SELECT id,nomor_spm,nama_sub_skpd,jenis_spm,nilai_spm FROM sipd.t_spm where id_user=1 AND like tanggal_spm=$start between tanggal_spm=$end";
    $sql = "SELECT a.id_spm as id,a.nomor_spm,b.nama_opd as skpd,a.jenis,a.nilai_spm FROM tspm a, skpd b, tspmsub c where c.updateby BETWEEN '$start' AND '$end' AND a.id_skpd=b.id_sipd AND a.id_spm=c.id_spm AND c.id_user=$id_user AND id_skpd=$opd";
    $result = mysqli_query($conn, $sql);
    $data = [];
    while ($row = mysqli_fetch_assoc($result)) {
      $data[] = $row;
    ...
    $end = $conn->real_escape_string($end);
    //menampilkan rekapan sumber dana
    $sql = "SELECT namasumberdana, sum(nilai_sumber) as nilai, sum(realisasi) as realisasinya, sum(sisa) as sisanya from (select c.id,a.namasubsumberdana,c.nilai_sumber,(SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(y.updateby) between '$start' AND '$end' AND z.id_spm=y.id_spm AND  y.id_sumber=c.id_subsumberdana AND y.status=1 AND z.id_skpd=$idopd) as realisasi,(SELECT COALESCE(sum(x.nilai_spm),0) from tspm x ,tspmsub w where Date(w.updateby) between '$start' AND '$end' AND x.id_spm=w.id_spm AND w.id_sumber > 0 AND x.id_skpd=b.id_sipd AND w.status=1) as totalrealisasi,b.nama_opd,c.nilai_sumber- (SELECT COALESCE(sum(z.nilai_spm),0) from tspm z, tspmsub y where Date(y.updateby) between '$start' AND '$end' AND z.id_spm=y.id_spm AND y.id_sumber=c.id_subsumberdana AND z.id_skpd=b.id_sipd AND y.status=1) as sisa,(select sum(nilai_sumber) from t_opdsumberdana where id_opd=b.id ) as Pagu_Anggaran, f.namasumberdana FROM subssumber a, skpd b, t_opdsumberdana c, pagu d, t_perubahan e ,t_sumberdana f where c.id_opd=b.id AND c.id_subsumberdana=a.id AND c.id_opd=b.id AND d.idopd=c.id_opd AND a.idsumberdana=f.id AND e.id=c.id_perubahan AND e.status='AKTIF') as subquery group by namasumberdana";
    $result = mysqli_query($conn, $sql);
    $data2 = [];
    while ($row = mysqli_fetch_assoc($result)) {
      $data2[] = $row;
